...
def parse_open_access(article_elem: etree.Element, pmc_id: Optional[str]) -> Dict:
    """
    Parse open access information.

    Returns dict with is_open_access flag, PMC URL, and license info.
    """
    # If has PMC ID, paper is in PubMed Central (open access repository)
    is_open_access = pmc_id is not None

    # Build PMC URLs if available
    ...
    license_info = None
    article = article_elem.find('.//PubmedData/ArticleIdList')
    if article is not None:
        # Some PubMed records include license in attributes
        # This is best-effort - not all records have this
        pass
